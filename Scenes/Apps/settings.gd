...
func _ready():
	...
	#Creation of default wallpaper selection
	#Stands for default wallpaer directory if that wasnt clear
	var defwpDir = "res://Assets/Wallpapers/"
	for file in DirAccess.get_files_at(defwpDir):
		if !file.ends_with(".import"):
			var button = TextureButton.new()
			button.stretch_mode = TextureButton.STRETCH_SCALE
			button.ignore_texture_size = true
			button.custom_minimum_size = Vector2(90,160)
			button.texture_normal = load(defwpDir + file)
			button.pressed.connect(self.defaultWallpaperSelected.bind(defwpDir + file))
			
			$ScrollContainer/SettingList/WallpaperControl/ScrollContainer/WPSelContainer.add_child(
				button
			)
...
func defaultWallpaperSelected(path):
...
func changeWallpaper(path:String):
	if path.begins_with("res://"):
		phoneMenu.get_node("Wallpaper").texture = load(path)
		$ScrollContainer/SettingList/WallpaperControl/WPPrev.texture = load(path)
		...
		if path.ends_with(".png") or path.ends_with(".jpeg") or path.ends_with(".jpg"):
			...
			#print(path, img)
			if !img:
				$ScrollContainer/SettingList/WallpaperControl/WPPrev.texture = load(
					"res://Assets/Buttons/ExitUP.png")
		...
		print("I dunno what to do with this.")
...
func _on_reset_pressed():
	#Update Preview
	var defaultPath = "res://Assets/Backgrounds/OceanBG.png"
	var img = load(defaultPath)
	#var wallpaper = ImageTexture.create_from_image(img)
	$ScrollContainer/SettingList/WallpaperControl/WPPrev.texture = img #wallpaper
	
	#Update WP
	phoneMenu.get_node("Wallpaper").texture = img #wallpaper
	ConfigManager.config["wallpaperPath"] = defaultPath
